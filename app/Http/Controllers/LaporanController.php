...
class LaporanController extends Controller
{
  ...
  public function dataKelasPaketSoal(Request $request)
  {
    ...
    return Datatables::of($jawabs)
      ...
      ->addColumn('kota', function ($jawabs) {
        if ($jawabs->user) {
          return $jawabs->user->kota;
        } else {
          return 'kota';
        }
      })
  ...
  public function detailLaporanPeserta(Request $request)
  {
    if (auth()->user()->status == 'Mentor' or auth()->user()->status == 'Admin' or auth()->user()->status == 'Peserta') {
      $user = User::where('id', auth()->user()->id)->first();
      $peserta = User::where('id', $request->id_user)->first();
      ...
      return view('laporan.detailPeserta', compact('user', 'peserta', 'hasil_ujian', 'soal'));
    } else {
      return redirect()->route('home.index');
    }
  }
  ...
  public function pdfHasilUjianPerpeserta(Request $request)
  {
    $peserta = User::where('id', $request->peserta)->first();
    $jumlah_soal = Detailsoal::where('id_soal', $request->soal)->count();
    $soals = Detailsoal::where('id_soal', $request->soal)->where('status', 1)->get();
    $jawabs = Jawab::where('id_soal', $request->soal)->where('id_user', $request->peserta)->where('status', 1)->get();
    $jawabBenar = Jawab::where('id_soal', $request->soal)->where('id_user', $request->peserta)->where('status', 1)->where('score', '!=', '0')->get();
    $jawab_first = Jawab::where('id_soal', $request->soal)
      ->where('id_user', $request->peserta)
    ...
    $jumlah_jawaban_benar = Jawab::where('id_soal', $request->soal)
      ...
      ->first();
    $pdf = PDF::loadView('laporan.pdf.hasil_ujian', compact('jawabs', 'peserta', 'soals', 'jawabBenar', 'jumlah_jawaban_benar', 'jawab_first', 'jumlah_soal'));
